...
int main()
{
    ...
    ok &= expect(kmNet_mouse_move(12, -7) == KMNET_SUCCESS,
                 "mouse move acknowledgement");
    ok &= expect(kmNet_mouse_left(1) == KMNET_SUCCESS,
                 "mouse button acknowledgement");
    ok &= expect(kmNet_keydown(0x04) == KMNET_SUCCESS,
                 "keyboard acknowledgement");
    ok &= expect(kmNet_mouse_right(2) == KMNET_ERR_ARGUMENT,
                 "invalid button state rejected");
