...
def test_port(port: str) -> None:
    try:
        ...
        makxd = MakxdController(com_port=port, send_init=False, override_port=True)
        makxd.connect()
...
def run_tests() -> NoReturn:
    ...
    try:
        ...
        test_file: Path = package_dir.parent / "tests"
        ...
        print("\nRunning pytest output directly...\n")
...
def main() -> None:
    ...
    if args[0] == "--testPort" and len(args) == 2:
        test_port(args[1])
    elif args[0] == "--runtest":
        run_tests()
    else:
