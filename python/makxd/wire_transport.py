...
def _network_response_normalize(data: bytes, raw: bool) -> bytes:
...
class BleWireTransport:
    ...
    async def _connect(self) -> None:
        ...
        await self._client.start_notify(
            ...
            lambda _sender, data: self._rx.put(
                _network_response_normalize(bytes(data), False)
            ),
        )
    ...
    def write(self, data: bytes) -> int:
        ...
        self._wait(
            self._client.write_gatt_char(BLE_RX_UUID, data, response=True)
        )
        return len(data)
