...
class Keyboard:
    ...
    def string(self, text: str) -> None:
        ...
        self.transport.send_api(
            f'km.string("{_escape_double_quoted(text)}")',
    ...
    def init(self, dt_uframes: int | None = None) -> None:
        ...
        if dt_uframes is not None:
            payload = int(_dt_value(dt_uframes)).to_bytes(2, "little")
        self.transport.send_api(
            f"km.init({_dt_value(dt_uframes)})",
            ApiOpcode.KEY_INIT,
            ApiVerb.EXEC,
            payload,
        )
    ...
    def multi_down(self, keys: Sequence[KeyboardKey]) -> None:
        self._multi("multidown", ApiOpcode.KEY_MULTI_DOWN, keys)

    def multi_up(self, keys: Sequence[KeyboardKey]) -> None:
        self._multi("multiup", ApiOpcode.KEY_MULTI_UP, keys)

    def multi_press(self, keys: Sequence[KeyboardKey]) -> None:
        self._multi("multipress", ApiOpcode.KEY_MULTI_PRESS, keys)
