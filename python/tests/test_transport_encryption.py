...
def test_encrypted_command_uses_authenticated_com_frame(monkeypatch):
    ...
    frame, transaction_nonce = encryption.encode_command(b"km.version()\r\n")

    assert frame[:2] == b"\xDE\xAD"
    assert frame[4] == 0x03
    assert transaction_nonce == bytes(range(12))
    assert frame.hex() == (
        "dead2c00030100000102030405060708090a0b"
        "dd109bffab1b9627e9b764d777a57a9d"
        "0ff569e7a308cdf1c2dd4d1f5562"
    )
    payload = frame[5:]
