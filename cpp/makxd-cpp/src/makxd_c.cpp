...
static makxd::ApiProtocol convert_api_protocol(
    makxd_api_protocol_t protocol) {
    return protocol == MAKXD_API_MAK_API
        ? makxd::ApiProtocol::MAK_API
        : makxd::ApiProtocol::KM;
}
...
static makxd::ConnectionConfig convert_connection_config(
    ...
    const makxd::ApiProtocol protocol =
        convert_api_protocol(source.api_protocol);
    if (source.method == MAKXD_CONNECTION_COM) {
        return makxd::ConnectionConfig::com(
            source.com_port ? source.com_port : "", protocol, key);
    }
...
makxd_error_t makxd_get_output_device(
    ...
    try {
        ...
        route->route_mask = result->routeMask;
        route->mouse_uframes = result->mouseUframes;
...
makxd_error_t makxd_get_api_protocol(
    makxd_device_t* device, makxd_api_protocol_t* api_protocol) {
    if (!device || !api_protocol) {
        return MAKXD_ERROR_INVALID_PARAMETER;
    }
    *api_protocol =
        device->cpp_device->getApiProtocol() == makxd::ApiProtocol::MAK_API
        ? MAKXD_API_MAK_API
        : MAKXD_API_KM;
    return MAKXD_SUCCESS;
}
...
makxd_device_t* makxd_device_create_with_transport_protocol(
    bool encryption_enabled,
    const char* encryption_key_hex,
    makxd_api_protocol_t api_protocol) {
    if ((encryption_enabled && !encryption_key_hex) ||
        (api_protocol != MAKXD_API_KM &&
            api_protocol != MAKXD_API_MAK_API)) {
        return nullptr;
    }
    try {
        return new makxd_device(
...
makxd_device_t* makxd_device_create_with_protocol(
    makxd_api_protocol_t api_protocol) {
    if (api_protocol != MAKXD_API_KM &&
        api_protocol != MAKXD_API_MAK_API) {
        return nullptr;
    }
    try {
