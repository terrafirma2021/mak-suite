...
void SerialPort::close() {
	...
	// The listener acquires m_commandMutex while delivering a response,
...
std::future<std::string> SerialPort::sendTrackedMakApi(
	...
	record.reserve(1u + payload.size());
	record.push_back(static_cast<uint8_t>(opcode));
	record.insert(record.end(), payload.begin(), payload.end());

	TransportEncodedCommand encodedCommand;
	try {
		if (m_transportEncryption->enabled()) {
			encodedCommand = m_transportEncryption->encodeCommand(record);
		} else {
			encodedCommand.bytes.resize(5u + payload.size());
			...
			encodedCommand.bytes[4] = static_cast<uint8_t>(opcode);
			std::copy(payload.begin(), payload.end(), encodedCommand.bytes.begin() + 5);
		}
	} catch (...) {
		std::promise<std::string> promise;
			...
			if (m_transportEncryption->enabled()) {
				for (const auto& decoded : m_transportEncryption->decodeResponses(
					...
					processMakApiResponse(
						decoded.plaintext, &decoded.transactionNonce);
				}
				continue;
			}
			...
			if (!makApiBuffer.empty() ||
				...
				for (;;) {
					...
					processMakApiResponse(std::span<const uint8_t>(
						makApiBuffer.data() + 4u, payloadBytes + 1u));
					makApiBuffer.erase(
						makApiBuffer.begin(),
...
void SerialPort::processMakApiResponse(
	...
	const uint8_t opcode = response[0];
	...
	try {
		if (response.size() == 2u && response[1] == 0xffu) {
			pending->second->promise.set_exception(
				std::make_exception_ptr(
					std::runtime_error("MAK_API command was rejected")));
		} else {
			pending->second->promise.set_value(std::string(
				reinterpret_cast<const char*>(response.data() + 1u),
				response.size() - 1u));
		}
	} catch (...) {
	}
	...
	if (m_connectionIo->config.method != ConnectionMethod::COM) {
		...
		if (length >= 5u && input[0] == 0xDEu && input[1] == 0xADu &&
			(m_connectionIo->config.method == ConnectionMethod::BLE ||
			 input[4] == 0x03u)) {
			wire.assign(input + 4u, input + length);
		} else {
