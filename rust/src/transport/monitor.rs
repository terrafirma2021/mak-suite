...
pub(crate) fn monitor_thread(inner: Arc<TransportInner>) {
    loop {
        ...
        *inner.device_kinds.lock().unwrap() = None;
        ...
        loop {
            ...
            let result = match &connection {
                crate::types::ConnectionConfig::Com { port: Some(port) } => {
                    super::serial::establish_connection(port, inner.transport_encryption.as_deref())
                }
                crate::types::ConnectionConfig::Com { port: None } => {
                    Err(crate::error::MakxdError::NotFound)
                }
                _ => super::wire::establish_network_connection(
                    &connection,
