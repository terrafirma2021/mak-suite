...
mod tests {
    ...
    fn command_frame_round_trips_with_standard_ccm() {
        ...
        let (frame, nonce) = encryption
            .encode_command_with_nonce(b"km.version()\r\n", transaction_nonce)
            .unwrap();
        ...
        assert_eq!(
            &frame[19..],
            &[
                0xdd, 0x10, 0x9b, 0xff, 0xab, 0x1b, 0x96, 0x27, 0xe9, 0xb7, 0x64, 0xd7, 0x77, 0xa5,
                0x7a, 0x9d, 0x0f, 0xf5, 0x69, 0xe7, 0xa3, 0x08, 0xcd, 0xf1, 0xc2, 0xdd, 0x4d, 0x1f,
                0x55, 0x62,
            ]
        );
    }
    ...
    fn authenticated_response_requires_matching_transaction_nonce() {
        ...
        nonce[0] = 1;
        nonce[1..].copy_from_slice(&transaction_nonce);
        let mut ciphertext = b"km.version()\r\nkm.MAKXD\r\n>>> ".to_vec();
        let tag = encryption
            .cipher()
            .unwrap()
            .encrypt_in_place_detached(GenericArray::from_slice(&nonce), &aad, &mut ciphertext)
        ...
        let (plaintext, returned_nonce) = encryption
            ...
            .unwrap();
        assert_eq!(plaintext, b"km.version()\r\nkm.MAKXD\r\n>>> ");
