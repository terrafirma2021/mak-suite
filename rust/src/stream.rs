...
pub fn decode_controller_stream(
    record: &StreamInputRecord,
) -> Option<ControllerStreamState> {
    if record.kind != StreamKind::Controller || record.values.len() != 21 {
        return None;
    }
    let u16_at = |offset: usize| {
        u16::from_le_bytes(record.values[offset..offset + 2].try_into().unwrap())
    };
...
mod tests {
    ...
    fn decodes_controller_tuple() {
        let record = StreamInputRecord {
            ...
            values: vec![
                5, 0, 0, 0, 2, 100, 0, 200, 0, 255, 255, 2, 0, 253, 255,
                4, 0, 251, 255, 6, 0,
            ],
        };
        assert_eq!(
